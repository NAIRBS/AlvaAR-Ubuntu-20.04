...
async function main(Module, Stats, ARSimpleView, ARSimpleMap, Video, THREE, isLiveMode = false) {
  ...
  let image_width = 480;  // ESP32 resolution
  let image_height = 320; // ESP32 resolution
  ...
  async function demoStream() {
    ...
    // Fetch the ESP32 YAML file and set it in the WASM module
    const yamlResponse = await fetch('./assets/dual_esp32.yaml');
    ...
    try {
      ...
      console.log('Parsed ESP32 left camera intrinsics:', leftCameraIntrinsics);
    } catch (e) {
      console.error('Failed to parse calibration YAML:', e);
    }
    
           // Persistent buffer allocation for ESP32 resolution
     const leftFrameSize = 480 * 320 * 4;  // ESP32 resolution
    ...
    alva = {
      ...
      getFrameKeypoints() {
        // NOTE: This function returns 2D keypoints and should NOT be used for left frame display
        // Left frame should only show triangulated 3D points via getFramePoints()
    ...
    const camRenderer = new ARSimpleView($view, 480, 320, sceneVisualizer);
    ...
    function render() {
      ...
      if (!document.hidden) {
        ...
        if (frameLeft && frameRight) {
          ...
          ctx.fillText("ESP32 Left Camera (Stereo)", 10, 18);

          if (showRightCamera) {
            ctx.fillStyle = "rgba(0,0,0,0.6)";
            ctx.fillRect(image_width + spacing, 0, 120, 25);
            ctx.fillStyle = "white";
            ctx.fillText("ESP32 Right Camera", image_width + spacing + 10, 18);
          }
          ...
          // NOTE: This is the ONLY way points should be displayed on left frame - triangulated 3D points only
