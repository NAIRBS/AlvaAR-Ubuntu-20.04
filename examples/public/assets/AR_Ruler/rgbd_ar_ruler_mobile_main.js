...
async function main(Module, Stats, ARSimpleView, ARSimpleMap, Video, THREE, isLiveMode = false, PerformanceMonitor = null) {
  ...
  // Three.js AR rendering setup for mobile distortion
  ...
  async function demoStream() {
    ...
    // Initialize AR renderer with distortion shader
    initARRenderer();
  ...
  if (visualizerPanel) {
    ...
    function render() {
      ...
      if (!document.hidden) {
        if (isLiveMode) {
          // Live mode: Use UNDISTORTED frames for AR processing (separate from display)
          ...
          // Video mode: Get frames from video elements - use UNDISTORTED frames for AR processing
        ...
        if (frameLeft && frameRight) {
          ...
          // 2. Draw AR overlays (3D points, markers, measurement panels, plane outlines) on composite canvas
          if (pose && arRulerSystem) {
            // Get camera intrinsics for projections
            ...
            // Draw 3D stereo frame points projected onto composite canvas
            if (alva.getFramePoints) {
                ...
                for (const point3D of points3D) {
                  if (point3D.z > 0.1) {
                    ...
                    if (x >= 0 && x < 640 && y >= 0 && y < 480) {
                      ...
                      if (point3D.z < 1.0) {
                        compositeCtxLeft.fillStyle = `rgb(255, ${intensity}, ${intensity})`;
                      } else if (point3D.z < 2.0) {
                        compositeCtxLeft.fillStyle = `rgb(${intensity}, 255, ${intensity})`;
                      } else {
                        compositeCtxLeft.fillStyle = `rgb(${intensity}, ${intensity}, 255)`;
                      }
                      
                      const size = Math.max(1, Math.floor(4 * (1.0 - depth)));
                      compositeCtxLeft.fillRect(x - size/2, y - size/2, size, size);
                    }
                  }
                }
              }
            ...
            // Draw measurement panel at top-right if start marker exists
            if (arRulerSystem.startPoint) {
              ...
              const margin = 10;
              const panelX = 640 - margin - panelWidth;
              const panelY = margin;
              
              compositeCtxLeft.fillStyle = 'rgba(0, 0, 0, 0.9)';
              compositeCtxLeft.fillRect(panelX, panelY, panelWidth, panelHeight);
              
              compositeCtxLeft.strokeStyle = '#00ff00';
              compositeCtxLeft.lineWidth = 3;
              compositeCtxLeft.strokeRect(panelX, panelY, panelWidth, panelHeight);
              
              compositeCtxLeft.save();
              compositeCtxLeft.fillStyle = '#00ff00';
              compositeCtxLeft.font = 'Bold 24px Arial';
              compositeCtxLeft.textAlign = 'center';
              compositeCtxLeft.textBaseline = 'middle';
              compositeCtxLeft.fillText(`${distance.toFixed(3)}m`, panelX + panelWidth/2, panelY + panelHeight/2);
              compositeCtxLeft.restore();
            }
            
            // Draw detected plane outline if plane mode is enabled
            if (arRulerSystem.usePlaneMode && arRulerSystem.detectedPlane) {
              drawPlaneOutlineOnFrame(compositeCtxLeft, arRulerSystem.detectedPlane, pose, leftCameraIntrinsics);
            }
          }
          
          // 3. Capture AR overlay from ARSimpleView renderer and composite it
          ...
          // 4. Update textures (composite will be distorted by shader)
